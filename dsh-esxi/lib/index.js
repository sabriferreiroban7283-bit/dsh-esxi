...
async function apply(ctx, rawConfig) {
	...
	ctx.systemPrompt.section({
		name: "tool:esxi",
		order: 108,
		text: "The esxi_* tools manage ESXi hosts, vCenter Server, and the vSphere ecosystem through the official govc CLI. Start with esxi_doctor (verify the govc binary; install: true downloads it), esxi_connect (add a connection profile), esxi_about (confirm connectivity), then esxi_inventory (map the environment). Destructive operations ask for approval through the session. Field-tested quick recipes (copy the shape, adjust names): create a VM: esxi_vm_create {name, cpu, memory, disk: '20GB', datastore, guestId, network, powerOn: false} then esxi_vm_power on; one-call shortcut esxi_vm_quick {name, cpu, memory, disk, datastore, iso?, cloudinit?, serial?, powerOn} covers create + ISO attach + cloud-init seed + serial console in a single invocation; add disk/snapshot: esxi_vm_disk {operation: create, name, size} (datastore defaults to the VM's own) and esxi_vm_snapshot {operation: create, name}; console access: esxi_vm_serial {operation: add} + {operation: connect, uri: '-'} (boot log) or uri: 'telnet://:PORT' (interactive, connect with nc); one-click OS install: esxi_vm_import {file: <ova>} + esxi_vm_cloudinit {vm, userData: '#cloud-config…'} BEFORE first power-on + esxi_vm_disk resize + power on. Field notes: standalone hosts without the clone right reject esxi_vm_clone (use export/import instead); hosts inside dot-named folders break absolute pool paths — use the glob form '*/Resources/<pool>'; manual NIC MACs must avoid VMware OUIs (00:0c:29); CD-ROM inserts auto-connect; cloud-init 26.x needs hashed_passwd (legacy passwd: silently ignored) and usernames that do not collide with existing groups (the images ship an 'admin' group). Safety: never test on production VMs (use tst-* names); a rejected stored password latches the target for 15 minutes — STOP retrying (retries are what lock ESXi accounts, typically at 5 failures) and update the password via esxi_connect. Field-tested techniques live in the plugin's SKILLS.md."
	});
